...
## 上下文
我现在实现了一个AI原生的广告网络系统，它的接入方式为
  @ads-plugin/CHATBOX_INTEGRATION.md  ,支持的广告格式详细内容为
  @ads-plugin/COMPONENT_DOCUMENTATION.md
  ，现在我想把本chatbox项目作为我的广告网络系统的试验田供我调试和客户体验。要
  求所有的广告都是可以在页面上配置的，比如我可以打开action_card卡片的广告形式
  并配置它的大小展示位等信息，在向广告平台触发请求的时候我也可以设置要传入进去
  的内容，比如只有用户输入文本、或者是用户输入文本加响应、或者是完整的用户上下
  文、或者是用户记忆、画像等信息都可以设置传入。
  你需要帮我分析评估一下这个方案的可行性和并设计架构和实现路径
